...
private:
  ...
  edm::EDGetTokenT<edm::ValueMap<unsigned int> > eleIdbitmapToken_;
...
ElectronNtuplerVIDDemo::ElectronNtuplerVIDDemo(const edm::ParameterSet& iConfig):
  ...
  eleIdbitmapToken_(consumes<edm::ValueMap<unsigned int> >(iConfig.getParameter<edm::InputTag>("eleIdBit"))),
...
ElectronNtuplerVIDDemo::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  edm::Handle<edm::ValueMap<unsigned int> > ele_id_bit;
  iEvent.getByToken(eleIdbitmapToken_ ,ele_id_bit);
  ...
  for (size_t i = 0; i < electrons->size(); ++i){
    printf("ele size---->>> %i", electrons->size());
    const auto el = electrons->ptrAt(i);
    printf("ele pt---->>> %f", el->pt());
